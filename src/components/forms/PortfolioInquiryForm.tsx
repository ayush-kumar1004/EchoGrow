...
import { submitPortfolioInquiry } from "@/actions/contact";
...
export default function PortfolioInquiryForm() {
  ...
  async function handleSubmit(formData: FormData) {
    startTransition(async () => {
      const result = await submitPortfolioInquiry(formData);
      if (result.success) {
        toast.success(result.message);
        setIsSubmitted(true);
        const formElement = document.getElementById("portfolio-inquiry-form") as HTMLFormElement;
    ...
    return (
      <div className="text-center py-6 px-4 bg-gray-50 rounded-2xl border border-gray-100 flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-xs text-gray-600 max-w-[200px] leading-relaxed">
          Thank you for your inquiry. Our team will review it and get back to you shortly.
        </p>
        <button 
          onClick={() => setIsSubmitted(false)}
          className="text-xs text-gray-500 hover:text-black transition-colors underline cursor-pointer mt-1"
        >
  ...
  return (
    <form action={handleSubmit} className="flex flex-col gap-3" id="portfolio-inquiry-form">
      <input 
      ...
      />
      <textarea
        className="w-full border border-gray-200 rounded-2xl px-5 py-3 text-sm focus:ring-black focus:border-black placeholder-gray-400 min-h-[80px] resize-none" 
        name="message"
      ...
      <button 
        ...
        type="submit"
      >
        {isPending ? "Submitting..." : "Get Free Audit"}
      </button>
    </form>
  );
}
